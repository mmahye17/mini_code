"""
Cron Scheduler — durable cron job scheduling.
...
from shared.config import CRON_DURABLE_PATH
...
def save_durable_jobs():
    ...
    CRON_DURABLE_PATH.write_text(json.dumps(durable, indent=2))


def load_durable_jobs():
    if not CRON_DURABLE_PATH.exists():
        return
    try:
        for item in json.loads(CRON_DURABLE_PATH.read_text()):
            job = CronJob(**item)
